...
class HomePageController extends GetxController with WidgetsBindingObserver {
  ...
  final RxMap<String, String?> dtcRelatedSensors = RxMap<String, String?>({});
  ...
  Future<void> _resendPendingSessions() async {
    ...
    for (final draft in drafts) {
      ...
      final datafileName = draft['datafileName'] as String?;
      final esnId = draft['esnId'] as int?;
      final dongleId = draft['dongleId'] as int?;
      ...
      if (datasetType == null ||
        ...
        await PendingSessionStorage.removeDraft(key);
        _log(
            'Discarded invalid draft ($key) — missing dataset, ESN, or dongle id, nothing valid to resend');
        continue;
      }
  ...
  Future<void> _sendPartialSessionReport(String reason) async {
    ...
    if (_resolvedDatasetType == null ||
        ...
        _dongleDbId == null) {
      _log(
          'Skipping session report ($reason) — dataset_type/datafile_name/dongle id not resolved yet, nothing valid to send');
      return;
    }
    _sessionReportSent = true;
...
// ✅ fresh session — nothing persisted until flashing starts
  ...
  void _resetForEsnEdit() {
    ...
    dtcList.clear();
    dtcRelatedSensors
        .clear(); // avoid stale sensor data leaking into a new ESN session
    pidList.clear();
    iqaWriteStatus.value = '';
    dtcReadStatus.value = '';
...
// ✅ reset so this session's flag doesn't leak forward
  ...
  Future<void> flushPendingSessionOnExit() async {
    ...
    if (_resolvedDatasetType == null ||
        ...
        _dongleDbId == null) {
      print(
          'Skipping exit-flush ($key) — dataset_type/datafile_name/dongle id not resolved, would be rejected by server');
      return;
    }
    _sessionReportSent = true;
...
//         if (_dongleDbId == null) {
//           _log(
//               '❌ Cannot send session report — dongle id not resolved (was the dongle actually matched to this vehicle?)');
//           flashInProgress.value = false;
//           return;
//         }
...
// kept for any UI state that reads it
        ...
        // ✅ Flash succeeded — record it as Pass IMMEDIATELY.
        // This must happen right here, not at the end of this function,
        // because IQA write / DTC read below can fail and trigger an early
        // _sendPartialSessionReport() + return, which would skip the block
        // at the bottom of this function that used to be the only place
        // _draftFlashStatus was set to 'Pass'. That bug caused a genuinely
        // successful flash to be reported to the server as 'Fail' whenever
        // a later step (IQA/DTC) failed.
        flashComplete.value = true;
        _log('Flashing completed successfully (${formattedElapsed})');
        _draftFlashStatus = 'Pass';
        ...
        final flashPassed = result == 'NOERROR';
        // Redundant now (already set to 'Pass' above the moment flashing
        // succeeded), but harmless to leave — it just re-confirms the same
        // value when the full sequence runs to completion without an early
        // exit.
        _draftFlashStatus = flashPassed ? 'Pass' : 'Fail';
        final iqaPassed = iqaWriteStatus.toLowerCase().contains('successful');
        final startTime = _flashCycleStartTime ?? DateTime.now();
  ...
  // Internal implementation — used directly by startFlashing(), which is
  // ALREADY inside a _withDongleBusy() block. Calling _loadDtcResults()
  // (the locking wrapper) from there would deadlock the app waiting on
  // its own outer lock, so the flashing flow calls this unlocked version
  // instead.
  Future<void> _loadDtcResultsUnlocked() async {
    final datasetId = _currentDtcDatasetId;
    if (datasetId == null) {
...
// Add this near the top of the class that contains _showActivityFullScreen(),
// alongside your other fields (e.g. next to _tagChip / _parseLogEntry / _activityLogColor).
  List<String> allTags = [
    'All',
    'ESN',
    'FLASH',
  ...
  String _inferLogTag(String message) {
    ...
    if (m.contains('session started')) return 'SESSION';
    if (m.contains('dongle')) return 'DONGLE';
    return 'All'; // ← default/fallback tag
  }
}
