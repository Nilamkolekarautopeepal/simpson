...
//               ),
//             )..interceptors.add(ApiLogInterceptor())); // ← add this line

//   /// Calls the login endpoint (multipart/form-data) and returns the
//   /// parsed [User] on success.
//   Future<User> login({
...
//       if (data is Map) {
//         serverMessage = (data["detail"] ?? data["message"])?.toString();
...
class AuthService {
  ...
  Future<User> login({
    ...
    try {
      ...
      String? serverMessage;
      if (data is Map) {
        // Include "error" alongside "detail"/"message" — this server
        // returns errors like {"error": "This device not approved",
        // "is_active": false} using the "error" key specifically, which
        // was previously not checked here, so genuine server reasons
        // (e.g. device-not-approved) were silently swallowed and replaced
        // by the generic "Invalid username or password." fallback below.
        serverMessage =
            (data["detail"] ?? data["message"] ?? data["error"])?.toString();
      } else if (data is String && data.trim().isNotEmpty) {
        try {
