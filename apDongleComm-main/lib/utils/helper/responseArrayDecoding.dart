      ...
      while (!endOfPacket) {
        ...
        if (responseBytes[nextPacketIndex] == 0x20) {
          ...
          // Negative Dongle Ack
            ...
            endOfPacket = true;
            firstposdongleackreceived = false;
            // ✅ FIX: default case added so unmapped dongle-error bytes
            // don't silently leave status at its initial "NOERROR" value.
            switch (responseBytes[3]) {
              case 0x10:
                status = "DONGLEERROR_COMMANDNOTSUPPORTED";
                break;
      ...
      while (!endOfPacket) {
        ...
        if (responseBytes[nextPacketIndex] == 0x20) {
          ...
          // Positive Dongle Ack (0x01 for Channel variants)
            ...
            endOfPacket = true;
            firstposdongleackreceived = false;
            // ✅ FIX: default case added — this is the exact switch that was
            // silently leaving status = "NOERROR" whenever responseBytes[4]
            // didn't match one of the 7 hardcoded dongle-error bytes (e.g.
            // the 0x35 byte seen in the "20 02 00 01 35 48 C8" frame that
            // was being logged as NOERROR after every single command).
            switch (responseBytes[4]) {
              case 0x10:
                status = "DONGLEERROR_COMMANDNOTSUPPORTED";
                break;
      ...
      while (!endOfPacket) {
        ...
        if (responseBytes[nextPacketIndex] == 0x20) {
          ...
          // Negative Dongle Ack
            ...
            endOfPacket = true;
            firstposdongleackreceived = false;
            // ✅ FIX (THE BUG): this switch previously had no default case.
            // responseBytes[4] == 0x35 (as seen live in your flash log for
            // the "20 02 00 01 35 48 C8" frame after EVERY command) matched
            // none of the 7 explicit cases below, so `status` was left at
            // its function-entry value of "NOERROR" — meaning every command
            // sent over a channel connection was reported as succeeding
            // regardless of what the dongle actually said. Added a default
            // that maps to SENDAGAIN so unrecognized dongle-ack bytes force
            // a retry/resend instead of silently passing as success.
            switch (responseBytes[4]) {
              case 0x10:
                status = "DONGLEERROR_COMMANDNOTSUPPORTED";
                break;
      ...
      while (endOfPacket == false) {
        if (responseBytes[nextPacketIndex] == 0x7F) {
          if (responseBytes[nextPacketIndex + 2] == 0x78) {
            ...
            // ECU negative response
            // ✅ FIX: default case added — same silent-NOERROR bug pattern
            // as the dongle-ack switches above. Without it, an unmapped NRC
            // byte here would leave `status` at its "NOERROR" initial value
            // even though the ECU explicitly returned a 0x7F negative
            // response frame.
            switch (responseBytes[nextPacketIndex + 2]) {
              case 0x10:
                status = "ECUERROR_GENERALREJECT";
